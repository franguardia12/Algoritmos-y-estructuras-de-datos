...
func Comparar(vector1 []int, vector2 []int) int {
	var menor_longitud int
	if len(vector1) < len(vector2) {
		menor_longitud = len(vector1)
	} else {
		menor_longitud = len(vector2)
	}
	for i := 0; i < menor_longitud; i++ {
...
func Seleccion(vector []int) {
	...
	var posicion_maximo int
	for i := len(vector); i > 0; i-- {
		posicion_maximo = Maximo(vector[:i])
		Swap(&vector[i-1], &vector[posicion_maximo])
	}
}
...
// Esta función debe implementarse de forma RECURSIVA. Se puede usar una función auxiliar (que sea
// la recursiva).
func _EsCadenaCapicua(cadena string, inicio int, fin int) bool {
	if inicio >= fin {
		return true
	}
	if cadena[inicio] == cadena[fin] {
		return _EsCadenaCapicua(cadena, inicio+1, fin-1)
	}
	return false
}
...
func EsCadenaCapicua(cadena string) bool {
	...
	inicio := 0
	fin := len(cadena) - 1
	return _EsCadenaCapicua(cadena, int(inicio), int(fin))
}
